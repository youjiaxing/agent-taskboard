...
| # | 结果 | 用户路径与证据 |
| --- | --- | --- |
| 1 空 Host 登记首个 Project | PASS | `project-registration.mjs`：从空 Host 的「登记 Project」进入，合法 Git remote 显示为候选；手工值保持不变，点击确认后才采纳；过期推断结果不会覆盖当前路径，失败保留草稿并可重试。 |
...
- Agent 选择页对未安装 Adapter 展示 command、搜索 PATH 与已知安装位置；动作禁用但不再静默。启动配置由各 Agent Adapter 探测本地 CLI 选项，并按 Project × Agent 在 10 分钟内复用；model 与对应 effort 可选，也保留手动输入兜底。
