...
| 尺寸 / Client | 结果 | 证据 |
| --- | --- | --- |
| 桌面 Release Tauri | **PASS** | 当前源码构建的 Release `.app`；实际点击 6 张 Issue，并反复打开/收起 Inspector。 |
...
- `tauri build --bundles app` 成功，bundle 使用当前实现重新生成；`codesign --verify --deep --strict` 通过。
- 在真实 Tauri WebView 中依次点击 #112、#113、#114、#115、#116，并在每张 Issue 上打开、收起 Inspector。
- 收起 #116 后，使用固定在顶栏的“显示详情”重新打开同一张 Issue；选择与 Inspector 上下文保持一致。
