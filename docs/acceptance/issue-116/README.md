...
| 桌面宽屏 Terminal 拖动、缩放、浮窗化 | **PASS** | 同一 Release Tauri；打开已有 Run，实际拖动 Terminal 从底栏转为浮窗，再从 `760 × 280` 缩放到 `845 × 324`。 |
...
| 重启恢复最近布局 | **PASS** | 关闭并重新打开同一 Release Tauri；Usage 恢复为 `838 × 620` 浮窗，Inspector 恢复为 `488 × 623` 停靠布局。 |
