...
未归档的活跃或已结束 Run 都可置顶。只有 `ended` Run 可以归档；活跃或等待操作 Run 的归档请求明确失败且不得停止进程。归档清除置顶，恢复只清除归档，不启动进程、不恢复 PTY、不改变执行状态。
...
归档不改变 Issue 认领、执行已停、Continue、自动推进、telemetry、隔离目录或历史关联。用量 totals 继续包含归档 Run 样本；pending confirmation 保留来源 Run ID 作为项目级非导航历史，可继续倒计时与 veto。
