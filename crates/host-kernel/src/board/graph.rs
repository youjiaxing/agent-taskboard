...
pub(super) fn dependency_overview(
    issues: &[IssueRecord],
...
pub(super) fn split_issue_id(id: &str) -> (String, u64) {
...
pub(super) fn lane(issue: &IssueRecord) -> Lane {
    if issue.claimed() {
        Lane::InProgress
...
pub(super) fn frontier_empty_reason(visible: &[&IssueRecord]) -> FrontierEmptyReason {
...
pub(super) fn card(issue: &IssueRecord, mapping_active: bool) -> IssueCard {
    IssueCard {
        id: issue.id(),
        repository: issue.repository.clone(),
        number: issue.number,
...
pub(super) fn issue_options(issues: &[IssueRecord]) -> Vec<IssueLink> {
...
pub(super) fn known_link_from_record(issue: &IssueRecord) -> IssueLink {
    IssueLink {
        id: issue.id(),
        repository: issue.repository.clone(),
...
pub(super) fn detail(issue: &IssueRecord, mapping_active: bool) -> IssueDetail {
    IssueDetail {
        id: issue.id(),
        repository: issue.repository.clone(),
        number: issue.number,
...
pub(super) fn link_detail(issue: &IssueRef) -> IssueDetail {
    IssueDetail {
        id: issue.id(),
        repository: issue.repository.clone(),
        number: issue.number,
...
pub(super) fn unclear_detail(repository: Option<&str>, number: Option<u64>) -> IssueDetail {
...
pub(super) fn known_link(issue: &IssueRef) -> IssueLink {
    IssueLink {
        id: issue.id(),
        repository: issue.repository.clone(),
        number: Some(issue.number),
...
pub(super) fn dependency_link(blocker: &DependencyRef) -> IssueLink {
    match blocker {
...
pub(super) fn unclear_id(repository: Option<&str>, number: Option<u64>) -> Option<String> {
