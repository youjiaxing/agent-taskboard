...
mod kernel_projection {
    ...
    fn title_search_stacks_with_triage_and_open_closed_filters() {
        ...
        tracker
            .add_issue(IssueRecord::open("you/garden", 2, "Fix search result").label("needs-info"));
        tracker.add_issue(
    ...
    fn recently_completed_uses_this_client_limit() {
        ...
        drop(host);
        let host =
            HostKernel::boot_with(boot_req(tmp.path()), Arc::new(MemoryTracker::new())).unwrap();
...
mod kernel_projection_edge_cases {
    ...
    fn centered_dependency_graph_expands_the_complete_upstream_and_downstream_closure() {
        ...
        tracker.add_issue(
            IssueRecord::open("you/garden", 99, "unrelated history")
...
mod browser_journeys_edge_cases {
    ...
    fn browser_supplements_issue_115_launch_form_behavior() {
        ...
        let grok =
            host_kernel::MemoryAgent::installed_grok().with_fields(fields.clone(), seed.clone());
        let codex = host_kernel::MemoryAgent::installed("codex", "Codex", "codex")
            .with_fields(fields, seed);
        let mut host = HostKernel::boot_with_ports(
            boot_req(tmp.path()),
            host_kernel::KernelPorts {
