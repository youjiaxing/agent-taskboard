...
**配对**:
Client 获准连上某个 Host 的一次性手续：交换可到达地址和一次性配对码，之后靠长期令牌。不是产品账号。Host 上可以撤销某个 Client。连通走用户自己的 Tailscale / VPN / 局域网。本机桌面窗口，以及打开 Host 自己端出来的回环页的本机浏览器，不必配对；其余入口要配对。
_Avoid_: 登录, 账号, 我们的中继

**自动更新**:
桌面应用向本仓库 GitHub Releases 检查新版本，人确认后再下载安装。不是后台偷换，也不走应用商店。有活跃 Run 时不装。
_Avoid_: 静默更新, 商店更新, 把检查更新当成登录

**Host 数据**:
这份 Host 在「这台电脑、这个系统用户」下的持久数据：已登记 Project、Run 结构化历史、按 Project×Agent 记住的启动配置默认、自动推进与 PATH 前缀、配对校验材料与已撤销名单，以及用户显式写入的 Tracker PAT。不是某个 Project 仓库里的文件，也不是 Client 设置。
_Avoid_: 应用数据（和 Client 设置分不清）, 用户目录, 把钥匙串当 Host 数据

**Client 设置**:
这个界面自己记的东西：界面语言、主题、最近完成 N、视图记忆、桌面进程开关（本机不起 Host、登录时启动）、已配对 Host 的地址与身份及长期令牌。本机桌面窗口和 Host 托盘共用一份；桌面应用、电脑浏览器、手机浏览器各记各的。
_Avoid_: 用户偏好（像账号）, 记在 Host 上共享, 跟随系统
...
**主题**:
某个 Client 上产品壳的一套外观。v1 清单：暖纸（仅白天）、素纸、素纸夜间。没有暖纸夜间。记在每个 Client，值是一份具体主题；设置里没有「跟随系统」。第一次按系统浅/深匹配：浅 → 暖纸，深 → 素纸夜间。不改变 Tracker 原文或官方 TUI。
