...
**Agent**:
本机安装的编码 Agent 命令行工具，必须能在 Embedded Terminal 里以官方交互 TUI 运行。v1 内置优先级：Grok Build → Codex → Claude Code → Antigravity CLI；名单不封闭，再接入一家是新的 Agent Adapter。Antigravity 只认 `agy`，Gemini CLI 不算另一家。只有本机 Web UI、没有官方交互 TUI 的不算。
_Avoid_: Bot, Assistant, LLM（模型只是 Agent 的配置维度之一）, 把 dsh web 这类浏览器入口先算进 Agent, 把 Gemini CLI 当成与 Antigravity CLI 并列的一家
...
**启动配置**:
某次启动 Run 时，用户在表单上确认过的、该 Agent Adapter 声明的那些值（如 model、effort、权限）。点启动时表单上是什么，这次 Run 就带着什么。按 Project×Agent 记住的那份默认不含隔离执行目录。另按 Project 记住上次成功启动用的是哪家 Agent；下次打开表单直接进那家，可换一家。
_Avoid_: RunConfig（假统一成三家同一套字段）, 把「用默认」当成用户看得见的值, 把是否隔离当成和 model 一样可记忆的偏好, 把上次用哪家 Agent 写进各家字段里

**Run 意图**:
启动一条新 Run 时可选的提示词快捷方式：修改、继续、只回答、复查。选中后生成可见、可编辑的开场白前缀。不是权限、Run 状态、Issue 状态、验收或自动推进条件。
_Avoid_: 任务类型, ACP session, 把意图当权限开关, 把「自定义」当成第五种意图
...
**Run 持久化恢复**:
Host 无法可靠读取 `runs.json` 时保留原文件并投影的结构化只读状态。此时历史不能当成空数组，所有会用不完整 Run 集合覆盖文件的操作都暂停；修复文件后由 Client 明确重试读取，完整加载成功才解除限制。
_Avoid_: 空历史, 自动重置 Run, 静默跳过坏记录, 用当前内存覆盖恢复证据
...
**最近完成**:
看板视图最右一列：只展示最近关闭的 N 张 Issue。N 默认 5，人可以改。用来找回刚关、还想再看的票。不是全部已关闭，也不是看板 Done 列；不能拖进这一列来关票。
_Avoid_: Done 列, 已关闭列, 完成列, 待审

**阻塞中**:
看板视图最左一列：未关闭、未认领、且仍有未完成 Dependency 挡着的 Issue。不是执行已停。
_Avoid_: 被挡住, Blocked（英文口令）, 把执行已停放进这列

**进行中**:
看板视图里已认领或有 Run 的未关闭 Issue 所在的列，含活跃 Run 与执行已停。
_Avoid_: 干活中, 执行中（会漏掉执行已停）

**看板视图**:
某个 Project 上按分组展示 Issue 的主视图：阻塞中、Frontier、进行中、最近完成。不是 Tracker 的开着/已关闭，也不能拖列来关票。
_Avoid_: 四列, Issue 板, 状态视图

**依赖图**:
某个 Project 上 Issue 之间 Dependency（谁挡谁）的图视图。只画挡住，不画父子。不是看板视图，也不是出厂默认的主界面（默认是看板）。
_Avoid_: 全图, 归属树, 相关图
