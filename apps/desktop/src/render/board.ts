...
export function workspaceRunHistory(
  ...
  return `<div class="workspace-run-history">${displayRuns.map((run) => {
    ...
    const pinMarker = run.pinnedAtMs != null
      ...
      : "";
    return `<article class="workspace-run-history-item ${run.pinnedAtMs != null ? "pinned" : ""}" data-act="focus-run" data-id="${escapeHtml(run.id)}" data-pinned="${run.pinnedAtMs != null}">
      <button type="button" class="workspace-run-history-main" data-act="focus-run" data-id="${escapeHtml(run.id)}">
        <span><b>${escapeHtml(run.agentName)}${pinMarker}</b><small>${escapeHtml(status)}</small></span>
